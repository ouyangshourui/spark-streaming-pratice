...
public class ResourcesUtil implements Serializable {
	...
	public static String getWebName() {
		String webName = "";
		String classPath = ResourcesUtil.class.getClassLoader().getResource("/").getPath();
		webName = classPath.substring(0, classPath.indexOf("/WEB-INF/classes"));
		webName = webName.substring(0, webName.lastIndexOf("/"));
		webName = webName.substring(0, webName.lastIndexOf("/"));
		webName = StringUtils.substringAfterLast(webName, "/");
		return webName;
	}
}
